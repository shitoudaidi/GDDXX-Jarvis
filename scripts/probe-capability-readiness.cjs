...
const child = childProcess.spawn(electronPath, [path.join(__dirname, "probe-capability-readiness-node.cjs")], {
  cwd: root,
  env: {
    ...process.env,
...
const timeout = setTimeout(() => {
  ...
  console.error(JSON.stringify({
    ok: false,
    error: "capability readiness probe timeout",
...
function parseResult() {
  ...
  const match = combined.match(/JARVIS_CAPABILITY_READINESS_RESULT\s+(\{[^\r\n]*\})/);
  if (!match) return;
  try {
...
child.on("exit", (code) => {
  ...
  if (!parsed) {
    console.error(JSON.stringify({
      ok: false,
      error: "capability readiness result marker missing",
      code,
      stdoutTail: stdout.split(/\r?\n/).filter(Boolean).slice(-30),
      stderrTail: stderr.split(/\r?\n/).filter(Boolean).slice(-30),
    }, null, 2));
