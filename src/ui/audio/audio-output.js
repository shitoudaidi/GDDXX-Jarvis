// audio-output.js — 贾维斯语音输出设备路由
//
// 背景 / 为什么需要这个模块：
//   贾维斯是"会说话"的助手，TTS 默认走 `new Audio()`，也就是听天由命跟着系统默认播放设备走。
//   而大众用户的机器上经常装着虚拟声卡（Steam 串流 / NVIDIA Virtual / VB-Audio / Pico VR 等），
//   或者拔掉耳机后 Windows 没有正确回退到内置扬声器 —— 系统默认就卡在一个"哑巴"虚拟设备上。
//   结果：贾维斯在说话，但用户什么都听不到，而且双方都不知道为什么。对语音产品这是致命的静默失败。
//
...
const OUTPUT_DEVICE_CACHE_MS = 5000;
let cachedOutputDevices = null;
let cachedOutputDevicesAt = 0;
let outputDevicesPromise = null;
...
export async function listOutputDevices({ force = false } = {}) {
  if (!navigator.mediaDevices?.enumerateDevices) return [];
  if (!force && cachedOutputDevices && performance.now() - cachedOutputDevicesAt < OUTPUT_DEVICE_CACHE_MS) {
    return cachedOutputDevices;
  }
  if (!force && outputDevicesPromise) return outputDevicesPromise;
  const task = navigator.mediaDevices.enumerateDevices()
    ...
    .then(devices => {
      ...
      cachedOutputDevicesAt = performance.now();
      return devices;
    })
    .finally(() => {
      if (outputDevicesPromise === task) outputDevicesPromise = null;
    });
  outputDevicesPromise = task;
  return task;
}
...
// 关键：贾维斯 TTS 只要 AudioContext 在运行，就会经 createMediaElementSource 走 Web Audio，
...
export async function applyContextSink(ctx) {
  ...
  if (ctx.__jarvisSink === want) return res; // 未变化 → 跳过，避免播放中重复切换造成爆音
  try {
    await ctx.setSinkId(want);
    ctx.__jarvisSink = want;
  } catch (err) {
    try { if (want) { await ctx.setSinkId(''); ctx.__jarvisSink = ''; } } catch {}
