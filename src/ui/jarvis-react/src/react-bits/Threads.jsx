...
export default function Threads({
  ...
  const propsRef = useRef({ color, amplitude, distance, enableMouseInteraction, maxFps });
  propsRef.current = { color, amplitude, distance, enableMouseInteraction, maxFps };
  ...
  useEffect(() => {
    ...
    const maxRenderDim = 1600;

    function resize() {
      const { clientWidth, clientHeight } = container;
      const baseDpr = Math.min(window.devicePixelRatio || 1, 1.5);
      const longestSide = Math.max(clientWidth, clientHeight) * baseDpr;
      const dpr = longestSide > maxRenderDim ? (baseDpr * maxRenderDim) / longestSide : baseDpr;
      renderer.dpr = dpr;
    ...
    function update(time) {
      ...
      const fps = Math.max(1, Math.min(60, Number(liveProps.maxFps) || 60));
      const frameInterval = 1000 / fps;
      if (time - lastRenderAt < frameInterval) return;
      lastRenderAt = time;

      program.uniforms.uColor.value.set(...liveProps.color);
      program.uniforms.uAmplitude.value = liveProps.amplitude;
      program.uniforms.uDistance.value = liveProps.distance;

      if (liveProps.enableMouseInteraction) {
