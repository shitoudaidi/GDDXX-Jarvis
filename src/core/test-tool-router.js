...
// ====== 5) TICK 广注入 ======
{
  ...
  // 按需求：core + web + memory + reminders + prefetch + ticker
  ...
  assert(!has(tools, 'hotspot_mode'), '5) TICK does not expose the retired hotspot panel')
...
// ====== 6) hasTask=true → 完整 task 控制组 ======
{
  ...
  assert(!has(tools, 'focus_banner'),
    '6) hasTask does not expose the retired focus banner')
}
...
// ====== 8) 中英混合：media 触发 ======
{
  ...
  assert(has(tools, 'music') && !has(tools, 'media_mode'),
    `8) music request exposes library management but not retired playback UI (got: ${tools.join(',')})`)
}
...
// ====== 11) 启动自检激活 ======
{
  ...
  assert(hasNone(tools, ['media_mode', 'hotspot_mode']),
    '11) startup self-check does not use retired panels')
}
...
// ====== 11b) Worldcup 触发 ======
{
  ...
  assert(!has(tools, 'worldcup_mode'),
    `11b) 世界杯赛况不注入已退役面板 (got: ${tools.join(',')})`)
...
}
{
  ...
  assert(has(tools, 'web_search') && !has(tools, 'worldcup_mode'),
    `11c) 比分追问走联网工具而非已退役面板 (got: ${tools.join(',')})`)
}
...
// ====== 14) Person card 触发 ======
{
  ...
  assert(has(tools, 'web_search') && !has(tools, 'person_card_mode'),
    `14) 人物问题走联网工具而非已退役人物卡 (got: ${tools.join(',')})`)
}
{
  const tools = selectTools({
    messageBody: '马云是谁',
  ...
  assert(has(tools, 'web_search') && !has(tools, 'person_card_mode'),
    `14b) 直接人物问题走联网工具 (got: ${tools.join(',')})`)
}
{
  const tools = selectTools({
