...
const SPEECH_VOL = Number(localStorage.getItem('jarvis-voice-speech-vol') || 0.016);
  const SILENCE_FINISH_MS = (() => {
    const v = parseInt(localStorage.getItem('jarvis-voice-finish-silence-ms') || '', 10);
    return Number.isFinite(v) && v >= 900 ? v : 2200;
  })();

export function createContinuousPolicy(core, { getAutoSend, getSingleTurn, finishVoiceTurn }) {
  // ─── 自动发送状态 ───
  ...
  let lastSpeechTs = 0;
  let silenceFinishTimer = null;
  let finishingBySilence = false;
  ...
  function cancelAutoSend() {
    ...
    if (silenceFinishTimer) { clearTimeout(silenceFinishTimer); silenceFinishTimer = null; }
  }

  function resetSpeechSilenceGate() {
    speechStarted = false;
    lastSpeechTs = 0;
    finishingBySilence = false;
    if (silenceFinishTimer) {
      clearTimeout(silenceFinishTimer);
      silenceFinishTimer = null;
    }
  }

  function scheduleSilenceFinishCheck() {
    if (silenceFinishTimer || finishingBySilence) return;
    const tick = () => {
      silenceFinishTimer = null;
      if (!speechStarted || finishingBySilence) return;
      if (!core.micActive || core.suspendedByMedia || core.pttHolding) return;
      if (getAutoSend?.() === false || getSingleTurn?.() === false) return;

      const quietFor = Date.now() - lastSpeechTs;
      const finishWindow = window.__JARVIS_INTERFACE_MODE__ === 'standby'
        ? Math.max(SILENCE_FINISH_MS, 2200)
        : SILENCE_FINISH_MS;
      if (quietFor >= finishWindow) {
        finishingBySilence = true;
        Promise.resolve(finishVoiceTurn?.({ reason: 'speech-silence' }))
          .finally(() => resetSpeechSilenceGate());
        return;
      }

      silenceFinishTimer = setTimeout(tick, Math.max(120, finishWindow - quietFor));
    };
    const initialWindow = window.__JARVIS_INTERFACE_MODE__ === 'standby'
      ? Math.max(SILENCE_FINISH_MS, 2200)
      : SILENCE_FINISH_MS;
    silenceFinishTimer = setTimeout(tick, initialWindow);
  }

  // ─── core 钩子：每帧音量 → barge-in 检测 + 活动计时 ───
  function onFrame(vol, frame = {}) {
    if (!core.suspendedByMedia && core.micActive && !core.pttHolding && getAutoSend?.() !== false && getSingleTurn?.() !== false) {
      if (vol >= SPEECH_VOL) {
        speechStarted = true;
  ...
  function onTranscript() {
    ...
    speechStarted = true;
    lastSpeechTs = Date.now();
    if (core.suspendedByMedia) return;
