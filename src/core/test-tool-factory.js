...
{
  const tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), `jarvis-tool-direct-perms-${suffix}-`))
  try {
...
{
  const tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), `jarvis-tool-factory-${suffix}-`))
  try {
...
{
  const tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), `jarvis-tool-legacy-${suffix}-`))
  try {
