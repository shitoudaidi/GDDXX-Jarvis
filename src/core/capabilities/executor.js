...
import { execUIHide, execUIShow, execUIUpdate, execUIPatch } from './tools/ui.js'
...
export async function executeTool(name, args, context = {}) {
  ...
  const eventId = `tool-${startedAt.toString(36)}-${crypto.randomBytes(3).toString('hex')}`
  const safeArgs = args || {}
  const policy = evaluateToolPolicy(name, safeArgs, context)
  const startedAtIso = new Date(startedAt).toISOString()

  emitEvent('tool_started', {
    id: eventId,
    tool: name,
    summary: summarizeToolReceipt(name, safeArgs),
    risk: policy?.risk || '',
  ...
  if (!policy.allowed) {
    ...
    writeToolAuditLog({ name, args: safeArgs, context, policy, status: 'denied', result, startedAt, eventId })
    return result
  }

  try {
    const result = await executeToolUnchecked(name, safeArgs, context)
    writeToolAuditLog({ name, args: safeArgs, context, policy, status: inferToolStatus(result), result, startedAt, eventId })
    return result
  } catch (err) {
    if (err.name === 'AbortError') throw err
    const result = `执行失败：${err.message}`
    writeToolAuditLog({ name, args: safeArgs, context, policy, status: 'error', result, error: err.message, startedAt, eventId })
    return result
  }
}

function summarizeToolReceipt(name, args = {}) {
  switch (name) {
    case 'read_file':
      return `读取文件：${args.path || args.filename || args.file_path || '?'}`
    case 'list_dir':
      return `列出目录：${args.path || args.dir || args.directory || '.'}`
    case 'write_file':
      return `写入文件：${args.path || args.filename || args.file_path || '?'}`
    case 'delete_file':
      return `删除文件：${args.path || args.filename || args.file_path || '?'}`
    case 'make_dir':
      return `创建目录：${args.path || args.dir || args.directory || '?'}`
    case 'exec_command':
      return `执行命令：${String(args.command || args.cmd || '?').slice(0, 100)}`
    case 'fetch_url':
    case 'browser_read':
      return `读取网页：${String(args.url || args.link || args.href || '?').slice(0, 120)}`
    case 'web_search':
      return `联网搜索：${String(args.query || args.q || args.keyword || '?').slice(0, 120)}`
    case 'search_memory':
    case 'recall_memory':
    case 'probe_memory':
      return `检索记忆：${String(args.query || args.q || args.keyword || name).slice(0, 120)}`
    case 'upsert_memory':
      return `写入记忆：${Array.isArray(args.memories) ? args.memories.length : 1} 条`
    case 'send_message':
    case 'express':
      return `发送消息：${args.target_id || '(unknown)'}`
    default:
      return `调用工具：${name}`
  }
}
...
function execFindTool({ query } = {}) {
  ...
  const ALWAYS_PRESENT = new Set(['find_tool', 'recall_memory', 'ui_show', 'ui_update', 'ui_hide', 'ui_patch'])
