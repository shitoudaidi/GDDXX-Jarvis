...
const ROLE_RULES = [
  {
    ...
    keywords: [/Jarvis/i, /agent/i, /llm/i, /prompt/i, /context/i, /memory/i, /codex/i, /claude/i, /openai/i, /minimax/i],
...
function inferProjects(text) {
  const projects = []
  if (/Jarvis/i.test(text) || /贾维斯/.test(text)) projects.push('Jarvis')
