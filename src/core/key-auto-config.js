...
const PROVIDER_RULES = [
  // TTS
  {
    ...
    makeConfig: (key) => ({
      configUpdates: { doubaoKey: key, doubaoResourceId: 'seed-tts-2.0' },
  ...
  },
  {
    ...
    makeConfig: (key) => ({
      configUpdates: { elevenLabsKey: key },
  ...
  },
  {
    ...
    makeConfig: (key) => ({
      configUpdates: { openaiTtsKey: key },
  ...
  },
  {
    ...
    makeConfig: (key, key2) => ({
      configUpdates: { volcanoToken: key, ...(key2 ? { volcanoAppId: key2 } : {}) },
...
const SEEDANCE_MODEL_TOKEN_RE = /\b(ep-[0-9a-z]{6,}|doubao-[a-z0-9-]{4,}|dreamina-[a-z0-9-]{4,}|seedance-[a-z0-9-]{2,})\b/ig

function detectSeedanceModel(text) {
  const raw = String(text || '')
  const explicit = raw.match(/(?:模型|model|endpoint|接入点)\s*[:：]?\s*(ep-[0-9a-z]{6,}|doubao-[a-z0-9-]{4,}|dreamina-[a-z0-9-]{4,}|seedance-[a-z0-9-]{2,})/i)
  if (explicit?.[1]) return explicit[1]
  SEEDANCE_MODEL_TOKEN_RE.lastIndex = 0
  let match
  while ((match = SEEDANCE_MODEL_TOKEN_RE.exec(raw)) !== null) {
...
export function detectSeedanceConfig(text) {
  ...
  // 先抓模型 ID（如有）。不要从 sk-... 这类密钥中间截出 seedance-* 当模型。
  const model = detectSeedanceModel(t)
...
export async function tryAutoConfigureKey(text, _recentContext = '') {
  ...
  // TTS：逐个测试并保存可用 key；当前发声 provider 仍保持本地 Jarvis。
