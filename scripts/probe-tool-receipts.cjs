...
const userDir = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-tool-receipts-"));
const child = childProcess.spawn(electronPath, [
  path.join(__dirname, "probe-tool-receipts-node.cjs"),
], {
  cwd: root,
  env: {
    ...process.env,
...
const timeout = setTimeout(() => {
  ...
  console.error(JSON.stringify({
    ok: false,
    error: "tool receipts probe timeout",
