...
async function fetchLocation() {
  ...
  const nominatim = await fetchJSON(
    ...
    { headers: { 'User-Agent': 'JarvisLocalAgent/1.0 (personal-assistant)' } },
    10000
  )
...
export function getGeoWeatherMeta() {
  const loc = _cached?.location || null
  return {
    countryCode: loc?.country_code ? String(loc.country_code).toUpperCase() : '',
